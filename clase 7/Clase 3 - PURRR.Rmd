...
La funcion `rnorm` toma los parámetros n (cantidad de observaciones), mu y sigma
...
`map(dataframe, funcion)`

De esta manera vamos a estar aplicando la función sobre todas las columnas del dataframe

```{r}
encuesta_sueldos_limpia = read_csv("~/Fuentes/encuesta_sueldos_it_limpia.csv")
```
...
Calculamos la mediana sobre las columnas numéricas, sabiendo que el resultado de la función es un número podemos usar la función `map_dbl`
...
El resultado de esto es una lista con los modelos
...
```{r}
modelos_por_perfil %>% map(summary) %>% map_dbl("r.squared")
```
