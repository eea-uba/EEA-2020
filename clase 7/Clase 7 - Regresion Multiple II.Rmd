...
En este caso para evaluar los modelos vamos a realizar una partición entre dataset de entrenamiento (70%) y testeo (30%) usando la función `initial_split` del paquete tidymodels.
...
Como $(SSTot/n−1)$ está fijo en un conjunto de datos dado (sólo depende de las Y observadas), el $R_a^2$ aumenta si y sólo si la $SSRes_p$ disminuye.
...
Vamos a efectuar el diagnóstico de los mejores dos modelos. 
...
## Predicción usando el dataset de testing

Si vamos a utilizar un modelo para predecir nuevos datos nos interesa observar el error del modelo el dataset de **training** y en el dataset de **testing** 

Vamos a emplear la función `augment` para predecir el sueldo neto sobre el dataset de testeo. Cuando se proporciona newdata, solo devuelve las columnas .fitted y .resid. 
...
Vamos a utilizar la función `rmse` de la librería [**yardstick**](https://yardstick.tidymodels.org/) de **tidymodels**. Los parámetros de la función son:
...
Podemos obtener los valores del RMSE en el dataset de **training** para los cuatro modelos evaluados
...
Esto crea una lista de 4 dataframes con los valores predichos. Luego podemos aplicar la función para calcular el RMSE de los cuatro modelos en el set de **training**
...
El modelo que obtiene el menor RMSE es el modelo **Años de experiencia y Nivel Educativo**. Veamos que sucede cuando obtenemos el RMSE de los modelos en el set de **testing** 
...
Ahora, el modelo que tiene el menor RMSE es el modelo **Años de experiencia y Género**
